...
export type ImportProgress = { jobId: string; rowsRead: number; bytesRead: number }
export type ImportReport = { accepted: number; duplicates: number; rejected: number }
...
export function decodeImportProgress(value: unknown): ImportProgress {
  ...
  return {
    ...
    rowsRead: number(raw.rowsRead, 'import progress.rowsRead'),
    bytesRead: number(raw.bytesRead, 'import progress.bytesRead')
  }
}

export function decodeImportResult(value: unknown): ImportResult {
  const raw = record(value, 'import result')
  const progress = raw.progress === undefined ? undefined : record(raw.progress, 'import result.progress')
  return {
    ...
    progress: progress ? {
      rowsRead: number(progress.rowsRead, 'import result.progress.rowsRead'),
      bytesRead: number(progress.bytesRead, 'import result.progress.bytesRead')
    } : undefined,
